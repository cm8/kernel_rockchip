...
/* The max erase timeout, used when host->max_discard_to isn't specified */
			...
			unsigned int to, unsigned int arg)
{
	 ...
	 * the erase operation does not exceed the max_discard_to, we should
	 * use R1B response. Or we need to prevent the host from doing hw busy
	 * detection, which is done by converting to a R1 response instead.
	 */
	if (card->host->max_discard_to &&
	    busy_timeout > card->host->max_discard_to) {
		cmd.flags = MMC_RSP_SPI_R1 | MMC_RSP_R1 | MMC_CMD_AC;
	} else {
		cmd.flags = MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;
		cmd.cmd_timeout_ms = busy_timeout;
		use_r1b_resp = true;
	}
					    ...
					    unsigned int arg)
{
	...
	unsigned int max_discard_to = host->max_discard_to ?
			host->max_discard_to : MMC_ERASE_TIMEOUT_MS;
	...
	/*
	 * We should not only use 'host->max_discard_to' as the limitation
	 * when deciding the max discard sectors. We should set a balance value
	 * to improve the erase speed, and it can not get too long timeout at
	 * the same time.
	 *
	 * Here we set 'card->pref_erase' as the minimal discard sectors no
	 * matter what size of 'host->max_discard_to', but if the
	 * 'host->max_discard_to' is large enough for more discard sectors,
	 * then we can continue to increase the max discard sectors until we
	 * get a balance value. In cases when the 'host->max_discard_to'
	...
	do {
		...
		for (x = 1; x && x <= max_qty && max_qty - x >= qty; x <<= 1) {
			...
			if (qty + x > min_qty && timeout > max_discard_to)
				break;
